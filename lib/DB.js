  ...
  if(sqlext.Objects && sqlext.Objects[moduleName] && sqlext.Objects[moduleName].length && _this.sql && _this.sql.object){
    ...
    for(var i=0;i<sqlobjects_table.length;i++){
      ...
      for(var j=0;j<sqlobjects_table_sorted.length;j++){
        if(sqlobjects_table_sorted[j].name in object_idx[tbl.name]._foreignkeys) max_fkey = j;
        else if(sqlobjects_table_sorted[j].name in object_idx[tbl.name]._dependencies) max_fkey = j;
      }
      max_fkey++;
    ...
    for(var i=0;i<sqlobjects_view.length;i++){
      ...
      for(var j=0;j<sqlobjects_view_sorted.length;j++){
        if(sqlobjects_view_sorted[j].name in object_idx[view.name]._tables){ max_tbl = j; }
      }
      max_tbl++;
    ...
    sqlobjects = sqlobjects_other.concat(sqlobjects_code).concat(sqlobjects_table_sorted).concat(sqlobjects_view_sorted);
...
DB.prototype.ParseSQL = function(sql, sqlFuncs){
  if(sqlFuncs) sql = this.ParseSQLFuncs(sql, sqlFuncs);
