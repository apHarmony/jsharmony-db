...
DB.prototype.RunScripts = function(jsh, scriptid, options, cb){
  ...
  function SQLNode(sql, module){
    this.sql = sql;
    this.module = module;
  }

  function findSQLScripts(node, scriptid, module){
    //Return current node if matched
    if(scriptid===null){
      if(_.isString(node)) return new SQLNode(node, module);
      var rslt = {};
      for(var key in node){
        rslt[key] = findSQLScripts(node[key], null, module);
      }
      return rslt;
    }
    else if(scriptid.length==0){
      return [findSQLScripts(node, null, module)];
    }
    //Search child nodes for scriptid
    var rslt = [];
    for(var key in node){
      var child = node[key];
      if((scriptid[0]=='*')||(scriptid[0]==key)) rslt = rslt.concat(findSQLScripts(child, scriptid.slice(1), (typeof module == 'undefined' ? key : module)));
    }
    return rslt;
  }
  ...
  async.eachSeries(dbscript_names, function(dbscript_name, db_cb){
    ...
    for(var i=0;i<sqlnodes.length;i++){
      ...
      if(jsh && jsh.Modules && sqlnode.module){
        var module = jsh.Modules[sqlnode.module];
        if(module){
...
DB.noDriver.prototype.getTableDefinition =
