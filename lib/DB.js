...
DB.prototype.getObjectSQL = function(jsh, sqlext, moduleName, module, sql, dbconfig){
  ...
  if(sqlext.Objects && sqlext.Objects[moduleName] && sqlext.Objects[moduleName].length && _this.sql && _this.sql.object){
    ...
    for(let i=0;i<sqlobjects_table.length;i++){
      var tbl = sqlobjects_table[i];
      var max_fkey = -1;
      for(let j=0;j<sqlobjects_table_sorted.length;j++){
    ...
    for(let i=0;i<sqlobjects_view.length;i++){
      var view = sqlobjects_view[i];
      var max_tbl = -1;
      for(let j=0;j<sqlobjects_view_sorted.length;j++){
