...
DB.prototype.Exec = function (context, sql, return_type, args){
  ...
  if (global.debug_params && global.debug_params.db_requests && global.log) {
    global.log.info(sql + ' ' + JSON.stringify(ptypes) + ' ' + JSON.stringify(params));
  }
...
DB.prototype.RunScripts = function(jsh, scriptid, options, cb){
  ...
  //Search SQLScripts tree for target scriptid
  var sqlscripts = findSQLScripts(jsh.SQLScripts, scriptid);
  var flatscripts = {};
  var dbscripts = [];
  ...
  var dbscripts = {};
  for(var i=0;i<flatkeys.length;i++) dbscripts[flatkeys[i]] = flatscripts[flatkeys[i]];
...
DB.prototype.RunScriptArray = function(jsh, dbscripts, options, cb){
  options = _.extend({ 
    ...
    onSQLResult: function(err, rslt){ },
    constring: global.dbconfig,
    sqlbase: undefined
  }, options||{});
  ...
  async.eachSeries(dbscript_names, function(dbscript_name, db_cb){
    ...
    if(options.sqlbase) sql = DB.ParseSQL(sql, options.sqlbase);
    sql = DB.ParseSQL(sql, jsh);
    var bsql = _this.sql.ParseBatchSQL(sql);
    var bi = 0;

    async.eachSeries(bsql, function(sql, sql_cb){
      bi++;
      options.onSQL(dbscript_name, bi, sql);
      var dbfunc = _this.Scalar;
      if(options.constring._driver.name=='sqlite'){
        dbfunc = _this.MultiRecordset;
        //global.debug_params.db_error_sql_state = true;
      }
      dbfunc.call(_this,'',sql,[],{},undefined,function(err,rslt){
        options.onSQLResult(err, rslt);
        dbrslt.push(rslt);
...
DB.log = function(txt){
  if(global.log) global.log.error(txt);
