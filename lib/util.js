...
function replaceSchema(sql, ent){
  if(!ent.Config || !ent.Config.schema_replacement) return sql;

  var schematokens = [];
  for(var i=0;i<ent.Config.schema_replacement.length;i++){
    var schema_replacement = ent.Config.schema_replacement[i];
    //Convert all functions to arrays of tokens
    try{ schematokens.push({ "name": schema_replacement.search_schema, "tokens": SQLScanner.Scan(schema_replacement.search_schema), "replace": schema_replacement.replace_schema }); }
    catch(ex) {  }
  }
...
exports.ParseSQL = function (sql, ent){
  ...
  if (ent){
    ...
    do{
      sql = newsql;
      newsql = replaceSchema(newsql, ent);
    } while(newsql != sql);
  }
  return sql;
}
