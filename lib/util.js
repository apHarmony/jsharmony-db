...
function applySQLFunc(sqlfunc, params, src, context){
  ...
  if(!variable_arguments && (params.length != sqlfunc.params.length)) throw new Error('SQL Function - invalid argument count: '+src);
  if(sqlfunc.sql){
    ...
    var f_config = [];
    f_exec = exports.ParseMultiLine(sqlfunc.exec);
    for(var i=0;i<sqlfunc.params.length;i++){
    ...
    var f = Function.apply(context, f_config);
      ...
      ex.message = 'Error compiling '+f_exec+' '+(ex.message||'');
      throw(ex);
    }
  ...
  if(ftokens.length){
    ...
    do{
      ...
      if(found_function_at !== null){
        var prev_last_token_index = last_token_index;
        last_token_index = i;
        ...
        for(var i=(tokenTo+1); i < tokens.length; i++){
  ...
  while(lastidx >= 0){
    ...
    if(fnameidx >= 0){
      ...
      if(sqlids[sqlid.toUpperCase()]){
        ...
        var sqlfunc = Macros[sqlids[sqlid.toUpperCase()]];
        if(sqlfunc && sqlfunc.params){
  ...
  if (sqlext){
    if(sqlext.Funcs){
      ...
      do{
        ...
        var rsltMacro = replaceMacro(newsql, sqlext.Funcs, options.log, context);
        newsql = rsltMacro.sql;
        var rsltFunc = replaceSQLFunc(rsltMacro.sql, sqlext.Funcs, context);
        newsql = rsltFunc.sql;
        //console.log('Time: '+(Date.now()-startTime)+' ms    Macro: '+rsltMacro.stats.count+' Func: '+rsltFunc.stats.count);
      } while(newsql != sql);
    }
