...
function replaceMacro(sql, Macros){
  var log = (global.log ? global.log.error : console.log);
...
exports.ParseSQL = function (sql, sqlbase){
  sql = exports.ParseMultiLine(sql);
  if (sqlbase){
    if(sqlbase.SQL){
      if(sql in sqlbase.SQL) sql = sqlbase.SQL[sql];

      var newsql = sql;
      do{
        sql = newsql;
        newsql = replaceMacro(newsql, sqlbase.SQL);
        newsql = replaceSQLFunc(newsql, sqlbase.SQL);
      } while(newsql != sql);
    }

    if(sqlbase.Config && sqlbase.Config.schema_replacement){
      do{
        sql = newsql;
        newsql = replaceSchema(newsql, sqlbase.Config.schema_replacement);
      } while(newsql != sql);
    }
  }
  return sql;
