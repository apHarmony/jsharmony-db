...
function applySQLFunc(sqlfunc, params, src, context){
  ...
  for(let i=0;i<sqlfunc.params.length;i++){
    let paramName = getParamName(sqlfunc.params[i]);
    if(paramName=='...'){ variable_arguments = true; continue; }
    if(params.length > i) continue;
    var defaultVal = 'null';
    if(sqlfunc.params[i] && sqlfunc.params[i].options && ('default' in sqlfunc.params[i].options)){
      defaultVal = sqlfunc.params[i].options.default;
      if((typeof defaultVal == 'undefined')||(defaultVal === null)) defaultVal = 'null';
      else defaultVal = defaultVal.toString();
    }
    ...
    for(let i=0;i<sqlfunc.params.length;i++){
      ...
      if(paramName=='...'){
        for(let j=i;j<params.length;j++){
          ...
          variable_arg += params[j];
        }
        if(!variable_arg) variable_arg = '';
      }
      else{
        let paramVal = params[i];
        if(sqlfunc.params[i] && sqlfunc.params[i].options && sqlfunc.params[i].options.trim){
          paramVal = (paramVal||'').toString().trim();
        }
        rslt = exports.ReplaceAll(rslt, '%%%'+paramName+'%%%', paramVal);
      }
    }
...
exports.ParseLOVValues = function(jsh, values){
  if(!values) return [];
  if(_.isArray(values)) return [].concat(values);
  var rslt = [];
